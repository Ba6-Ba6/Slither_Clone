...
export class Game {
  ...
  updateBotAI(bot, dt) {
    ...
    if (enemy && (bot.length > enemy.length * 0.82 || distance(head, enemy.head) < 420)) {
      ...
      if (this.config.projectile.enabled && aimError < 0.22 && Math.random() < this.config.bots.shootChance) {
        ...
        bot.input.shotCharge = Math.random() < 0.72 ? rand(0.05, 0.35) : rand(0.35, 0.8);
      }
      return;
    }
  ...
  tryShoot(snake, charge = 0) {
    if (!this.config.projectile.enabled) return false;
    const pconf = this.config.projectile;
    const c = clamp(Number(charge) || 0, 0, 1);
    const cost = Math.round(pconf.baseCost + pconf.chargeCost * c);
    if (!snake.alive || snake.length < this.config.snake.minLength + cost + 20 || snake.shotCooldown > 0) return false;

    snake.length -= cost;
    snake.shotCooldown = pconf.cooldown + c * 0.28;

    const scoreBonus = clamp(Math.sqrt(Math.max(0, snake.score)) * pconf.scorePowerScale, 0, pconf.maxScorePowerBonus);
    const power = (0.72 + c * 1.75) * (1 + scoreBonus);

    const start = {
      x: snake.head.x + Math.cos(snake.angle) * (snake.radius + 22),
      y: snake.head.y + Math.sin(snake.angle) * (snake.radius + 22)
    };
    ...
    for (const projectile of [...this.projectiles.values()]) {
      ...
      for (const b of bodies) {
        ...
        if (Math.hypot(projectile.x - b.x, projectile.y - b.y) < projectile.radius + b.radius) {
          b.snake.applyProjectileHit(b.index, projectile, this);
          const owner = this.snakes.get(projectile.ownerId);
          if (owner && owner.alive) {
            owner.score += Math.round(24 + projectile.charge * 42);
          }
          this.projectiles.delete(projectile.id);
          break;
        }
